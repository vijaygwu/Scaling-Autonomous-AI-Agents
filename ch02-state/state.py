...
class RedisStateStore(StateStore[dict[str, Any]]):
    ...
    async def set(
        ...
        pipe.setex(meta_key, ttl, json.dumps({
...
class PostgresTaskStore:
            ...
            if expected_version is not None:
                result = await conn.execute(
                    ...
                    WHERE task_id = $1 
                        AND attempt_count = $3
                    """,
                    task_id, json.dumps(checkpoint), expected_version
                )
            else:
                result = await conn.execute(
                    """
    ...
    async def transition_status(
        ...
        async with self._pool.acquire() as conn:
            completed_at = "NOW()" if new_status == TaskStatus.COMPLETED else "NULL"
            result = await conn.execute(
                f"""
                UPDATE task_state 
                SET status = $2,
                ...
                WHERE task_id = $1
                """,
                task_id, new_status.value, error_message
            )
            return result == "UPDATE 1"
...
class ImportanceScorer:
    ...
    def _extract_key_phrases(self, text: str) -> list[str]:
        ...
        words = text.split()
        return [" ".join(words[i:i+3]) for i in range(0, len(words)-2, 3)]
...
class CorruptionHandler:
    ...
    async def _partial_recovery(
        ...
        if recovered:
            ...
            recovered["_recovery_timestamp"] = datetime.now(timezone.utc).isoformat()
            return recovered
    ...
    def _record_corruption(self, key: str, corruption_type: str) -> None:
        ...
        if self._metrics:
            self._metrics.increment(
                "state.corruption",
                tags={"key_prefix": key.split(":")[0], "type": corruption_type}
            )
...
class SharedStateCoordinator:
    ...
    async def release_lock(
        ...
        result = await self._store._redis.eval(
            script, 1, lock_key, owner_id
        )
        return result == 1
...
class ConflictResolver:
        ...
        for key in all_keys:
            if key.startswith("_"):
                ...
                if isinstance(val_a, (int, float)) and isinstance(val_b, (int, float)):
                    ...
                    merged[key] = ConflictResolver.merge_additive(val_a, val_b)
                else:
    ...
    def vector_clock_merge(
        ...
        a_dominates = all(
            clock_a.get(k, 0) >= v for k, v in clock_b.items()
        )
        b_dominates = all(
            clock_b.get(k, 0) >= v for k, v in clock_a.items()
        )
